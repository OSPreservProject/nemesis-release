#'$Id: HTMLgen.py 1.1 Thu, 18 Feb 1999 14:18:34 +0000 dr10009 $'
...
class SeriesDocument(SimpleDocument):
    ...
    def nav_buttons(self):
        ...
        if self.goprev: # place an image button for previous page
            btn = Image(self.prev, border=0, alt=self.tagprev)
        ...
        if self.gonext: # place an image button for next page
            btn = Image(self.next, border=0, alt=self.tagnext)
        ...
        if self.gotop: # place an image button for top of manual page
            btn = Image(self.top, border=0, alt=self.tagtop)
        ...
        if self.gohome: # place an image button for site home page
            btn = Image(self.home, border=0, alt=self.taghome)
    ...
    def sub(self, source, subs=None):
        ...
        *subs* mapping of symbols to replacement values
        ...
        while matched:
            ...
            output.append(str(substitutions.get(source[i+a+1:i+b-1], source[i+a:i+b])))
            i = i + b
            matched = self.subpat.search(source[i:])
        else:
...
class Href:
    ...
    Keyword arguments
        ...
        onMouseOver -- is the script-code which is executed when the mouse
                       moves of the line.
        name -- is the optional name symbol
    """
    ...
    def __str__(self):
        s = ['<A']
        if self.url: s.append(' HREF="%s"' % self.url)
