 ...
 ** ENVIRONMENT: 
 **
 **      Module used by privileged domains.
 **
 ** NOTES:
 ** 
 **      Based almost entirely on the Linux driver (see copyright below)
 ** 
 ** ID : $Id: pci.c 1.1 Thu, 18 Feb 1999 14:16:19 +0000 dr10009 $
 ** 
 */
 ...
 */
const struct pci_dev_info dev_info[] = {
    ...
    DEVICE( DEC,                DEC_OPPO,       "DGLPB-AB"), /*NAS*/
    ...
    DEVICE( IDT,		IDT_IDT77201,	"IDT77201 NICStAR"),
...
const char *pci_strvendor(unsigned int vendor)
{
    ...
    case PCI_VENDOR_ID_IDT:     return "Integrated Device Technologies";
			 ...
			 unsigned char pos, int turn_on)
{
	...
	if (!bmap->addr) {
	    ...
	    pcibios_read_config_byte(st, bus, devfn, bmap->addr, &val);
	    if ((val & bmap->mask) == bmap->value) {
		printk("%s.", bridge_optimization[i].on);
		if (!turn_on) {
		    pcibios_write_config_byte(st, bus, devfn,
					      bmap->addr,
					      (val | bmap->mask)
					      - bmap->value);
...
/*
** Create stretches over any PCI memory regions, taking note of the 
** prefetchability or not [translated into cacheability on most archs].  
 */
static void map_dev_memory(struct pci_st *st, struct pci_dev *dev, 
			   FramesF_cl *framesF)
{
    u64 base;
    ...
    Mem_PMemDesc pmem; 
    uint32_t fw; 
    Mem_Attrs attrs = SET_ELEM(Mem_Attr_NonMemory);  /* means not DRAM */
    int reg;

    bus   = dev->bus->number;
    devfn = dev->devfn;

    pcibios_read_config_dword(st, bus, devfn, PCI_CLASS_REVISION, &class_rev);
    pcibios_read_config_word(st, bus, devfn, PCI_VENDOR_ID, &vendor);
    pcibios_read_config_word(st, bus, devfn, PCI_DEVICE_ID, &device);
    pcibios_read_config_word(st, bus, devfn, PCI_STATUS, &status);
    pcibios_read_config_byte(st, bus, devfn, PCI_BIST, &bist);
    pcibios_read_config_byte(st, bus, devfn, PCI_LATENCY_TIMER, &latency);
    pcibios_read_config_byte(st, bus, devfn, PCI_MIN_GNT, &min_gnt);
    pcibios_read_config_byte(st, bus, devfn, PCI_MAX_LAT, &max_lat);

    TRC(printk("  Bus %2d, device %3d, function %2d:\n",
	       bus, PCI_SLOT(devfn), PCI_FUNC(devfn)));
    
    TRC(printk("    %s: %s %s (rev %d).\n      ",
	       pci_strclass(class_rev >> 8), pci_strvendor(vendor),
	       pci_strdev(vendor, device), class_rev & 0xff));
    
    if (!pci_lookup_dev(vendor, device)) 
	TRC(printk("Vendor id=%x. Device id=%x.\n      ",  vendor, device));

    /* Check the format of the configuration space */
    pcibios_read_config_byte(st, bus, devfn, PCI_HEADER_TYPE, &hdr_type);

    if(hdr_type == 0x0) {
	unsigned int dw; 

	for (reg = PCI_BASE_ADDRESS_0; reg <= PCI_BASE_ADDRESS_5; reg += 4) {
	    pcibios_read_config_dword(st, bus, devfn, reg, &dw);

	    base = (dw == 0xffffffff) ? 0 : dw;
	    if (!base)
		continue;
	    
	    if (!(base & PCI_BASE_ADDRESS_SPACE_IO)) {  /* Got some memory */
		uint32_t size; /* asssume <= 4Gb */
		
		if (!(base & PCI_BASE_ADDRESS_MEM_PREFETCH)) 
		    attrs |= SET_ELEM(Mem_Attr_NoCache);

		if ((base & PCI_BASE_ADDRESS_MEM_TYPE_MASK) 
		...
		/* SMH: now write a value of 0xffffffff to the register
		   and read it back to determine the decoded size. 
		   Make sure we reinstall the original value too! */
		pcibios_write_config_dword(st, bus, devfn, reg, 0xffffffff);
		pcibios_read_config_dword(st, bus, devfn, reg, &dw);
		pcibios_write_config_dword(st, bus, devfn, reg, base);


		base &= PCI_BASE_ADDRESS_MEM_MASK;

		/* We OR in 0xF to the returned value since the 
		   bottom four bits are 'special'. */
		
		size = (~dw | 0xF) + 1; 
		TRC(printk("      memory at 0x%qx, size 0x%lx\n", base, size));

#define ALIGNED(_x,_fw) (!((word_t)(_x) & ((1UL<<(_fw))-1)))
#define ROUNDUP(_x,_fw) (((word_t)_x + (1UL<<(_fw))-1) & ~((1UL<<(_fw))-1))

		/* Determine the frame width based on alignment and size */
		for(fw = 0; fw <= WIDTH_MAX; fw++) {
		    if((!ALIGNED(base, fw+1)) || !ALIGNED(size, fw+1))
			break;
		}
		/* Now reduce width until get to a sensible size */
		while((fw >= WIDTH_MIN) && !VALID_WIDTH(fw)) 
		    fw--; 


		/* 
		** XXX SMH: should determine appropriate frame width 
		** here according to device (e.g. framebuffer should 
		** use largest fwidth since protection makes no sense
		** within the fb, but an ethernet card might want per
		** page protection or something) 
		*/

		TRC(printk("     Aligned to %d bytes.\n", fw));
		
		/* XXX SMH: assume we're going to be using a dense mapping
		   from host to PCI space for now. XXX */
		pmem.start_addr  = (addr_t)pci_dense_to_phys((word_t)base); 
		pmem.nframes     = size >> fw; 
		pmem.frame_width = fw; 
		pmem.attr        = attrs; 
		FramesF$AddFrames(framesF, &pmem);
	    }
	}
    } else {
	unsigned int dw1, dw2, dw3; 
	word_t smem_base, smem_limit; 
	u64 pmem_base, pmem_limit; 

	switch(hdr_type) {
	case 0x80: 
	    TRC(printk("      PCI-ISA Bridge => can't handle config area.\n"));
	    break;

	case 0x01: 
	    /* 
	    ** Header of 0x01 => configuration space from 0x10-0x3F 
	    ** conforms to the standard PCI-to-PCI bridge layout. 
	    */
	    TRC(printk("      PCI-PCI Bridge Configuration Space:\n"));

	    /* Get standard mem base and limit */
	    pcibios_read_config_dword(st, bus, devfn, 0x20, &dw1);
	    smem_base  = (dw1 & 0xFFF0) << 20;  
	    smem_limit = (dw1 & 0xFFF00000) << 4;  
	    if(smem_limit > smem_base) {
		/* Add region to frames */

		/* Determine the frame width based on alignment and size */
		for(fw = 0; fw < 32; fw++) {
		    if((!ALIGNED(smem_base, fw+1)) || 
		       !ALIGNED(smem_limit, fw+1))
			break;
		}
		...
		pmem.start_addr  = (addr_t)smem_base; 
		pmem.nframes     = (smem_limit - smem_base) >> fw; 
		pmem.frame_width = fw; 
		pmem.attr        = SET_ELEM(Mem_Attr_NoCache);
		FramesF$AddFrames(framesF, &pmem);
	    }

	    /* Get prefetchable mem base and limit */
	    pcibios_read_config_dword(st, bus, devfn, 0x24, &dw1);
	    pcibios_read_config_dword(st, bus, devfn, 0x28, &dw2);
	    pcibios_read_config_dword(st, bus, devfn, 0x2C, &dw3);
	    pmem_base  = ((u64)dw2 << 32) | ((dw1 & 0xFFF0) << 20);  
	    pmem_limit = ((u64)dw3 << 32) | ((dw1 & 0xFFF00000) << 4);  
	    
	    if(pmem_limit > pmem_base) {
		/* Add region to frames */
		
		/* Determine the frame width based on alignment and size */
		for(fw = 0; fw < 32; fw++) {
		    if((!ALIGNED(pmem_base, fw+1)) || 
		       !ALIGNED(pmem_limit, fw+1))
			break;
		}
		...
		pmem.start_addr  = (addr_t)(word_t)pmem_base; 
		pmem.nframes     = (pmem_limit - pmem_base) >> fw; 
		pmem.frame_width = fw; 
		pmem.attr        = 0;
		FramesF$AddFrames(framesF, &pmem);
	    }
	...
	default:
	    printk("PCIBios: weird header type 0x%x\n", hdr_type);
	    break;
	}
 ...
 */
void map_pci_devices(struct pci_st *st, FramesF_cl *framesF)
{
    struct pci_dev *dev;

    TRC(printk("\n\nmap_pci_devices (st at %p)\n", st));
    TRC(printk("PCI devices found:\n"));
    for (dev = st->pci_devices; dev; dev = dev->next)
	map_dev_memory(st, dev, framesF);
 ...
 */
static int sprint_dev_config(struct pci_st *st,struct pci_dev *dev, 
			     char *buf, int size)
{
    unsigned long base;
...
#		undef BIST_capable
    ...
    /* Check the format of the configuration space */
    pcibios_read_config_byte(st, bus, devfn, PCI_HEADER_TYPE, &hdr_type);
    if(hdr_type == 0x0) { /* only do devices here; */
	for (reg = PCI_BASE_ADDRESS_0; reg <= PCI_BASE_ADDRESS_5; reg += 4) {
	    if (len + 40 > size) {
		return -1;
	    }
	    pcibios_read_config_dword(st, bus, devfn, reg, &l);

	    base = (l == 0xffffffff) ? 0 : l;
	    if (!base) {
		continue;
	    }
 ...
 * BUF is a buffer that is PAGE_SIZE bytes long.
 */
int get_pci_list(struct pci_st *st, char *buf)
{
...
#	define MSG "\nwarning: page-size limit reached!\n"
    ...
    for (dev = st->pci_devices; dev; dev = dev->next) {
	nprinted = sprint_dev_config(st, dev, buf + len, size - len);
	if (nprinted < 0) {
...
static unsigned int scan_bus(struct pci_st *st, 
			     struct pci_bus *bus, 
			     unsigned long *mem_startp)
{
	...
	if (PCI_FUNC(devfn) == 0) {
	    pcibios_read_config_byte(st, bus->number, devfn,
	...
	pcibios_read_config_dword(st, bus->number, devfn, PCI_VENDOR_ID,
				  &l);
	...
	pcibios_read_config_byte(st, bus->number, devfn, PCI_COMMAND,
				 &cmd);
	pcibios_write_config_byte(st, bus->number, devfn, PCI_COMMAND,
	...
	pcibios_read_config_dword(st, bus->number, devfn,
	...
	if (dev->class >> 8 == PCI_CLASS_BRIDGE_PCI) {
	     ...
	     */
	    pcibios_read_config_word(st, bus->number, devfn,
				     PCI_COMMAND, &cr);
	    pcibios_write_config_word(st, bus->number, devfn,
				      PCI_COMMAND, 0x0000);
	    pcibios_write_config_word(st, bus->number, devfn,
				      PCI_STATUS, 0xffff);
	    /*
	     ...
	     */
	    pcibios_read_config_dword(st, bus->number, devfn, 0x18,
				      &buses);
	    buses &= 0xff000000;
	    buses |= (((unsigned int)(child->primary)     <<  0) |
		      ((unsigned int)(child->secondary)   <<  8) |
		      ((unsigned int)(child->subordinate) << 16));
	    pcibios_write_config_dword(st, bus->number, devfn, 0x18,
				       buses);
	    /*
	     * Now we can scan all subordinate buses:
	     */
	    max = scan_bus(st, child, mem_startp);
	    /*
...
unsigned long pci_init (struct pci_st *st,
			unsigned long mem_start, 
			unsigned long mem_end)
{
        mem_start = pcibios_init(st, mem_start, mem_end);
	...
	st->pci_devices = 0;
        memset(&st->pci_root, 0, sizeof(st->pci_root));
        st->pci_root.subordinate = scan_bus(st, &st->pci_root, &mem_start);

        /* give BIOS a chance to apply platform specific fixes: */
        mem_start = pcibios_fixup(st, mem_start, mem_end);
#ifdef DEBUG
        {
                int len = get_pci_list(st, (char*)mem_start);
                if (len) {
