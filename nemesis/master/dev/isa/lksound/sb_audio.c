...
{
	sb_devc *devc = audio_devs[dev]->devc;
	printk("sb16 audio set speed; devc is %x\n", devc);
	if (speed > 0)
	{
		if (speed < 5000)
			speed = 4000;
			...
			int intrflag)
{
	...
	printk("audio output block size %d count %d\n", cnt, count);
	save_flags(flags);
	cli();

	DMAbuf_start_dma (dev, buf, count, DMA_MODE_WRITE | DMA_AUTOINIT);
