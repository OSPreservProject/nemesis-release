...
#ifdef DEBUG
#define TRC(_x) _x;
#define TRCF(_x) fprintf(stderr,_x), PAUSE(SECONDS(1)/4);
#else
#define TRC(_x)
#define TRCF(_x)
#endif
...
#define BIOS_BASE  0xc0000
   ...
   static int init=0;
   int i,j,l1,l2;
   TRCF("Find BIOS string");
#ifdef INTEL
   if (!init) {
      init = 1;
      if (xf86ReadBIOS(addr /* was BIOSbase - why? */, 0, bios, BIOS_BSIZE) != BIOS_BSIZE)
	 return NULL;
...
s3Probe()
{
   ...
   TRCF("Calling xf86ClearIOPortList");
   ...
   TRCF("Let's do some IO, children...");
...
#undef Setcomm
   ...
   if (OFLG_ISSET(CLOCK_OPTION_TI3026, &s3InfoRec.clockOptions)) {
      ...
      if (xf86Verbose)
	 ErrorF("%s %s: Using TI 3026 programmable clock (MCLK %d MHz)\n",
		clockchip_probed, s3InfoRec.name, (int) (mclk ));
      numClocks = 3;
      if (!s3InfoRec.s3MClk)
	 ...
	 mcc = 1;
      ...
      if (xf86Verbose)
	 ErrorF("%s %s: Using TI 3025 programmable clock (MCLK %d KHz)\n",
		clockchip_probed, s3InfoRec.name, mclk);
      numClocks = 3;
      if (OFLG_ISSET(OPTION_NUMBER_NINE, &s3InfoRec.options)) {
	 mclk = 55000;
	 if (xf86Verbose)
	    ErrorF("%s %s: Setting MCLK to %d MHz for #9GXE64 Pro\n",
		   XCONFIG_PROBED, s3InfoRec.name, mclk);
	 ...
	 n1 = n0;
      ...
      {
	  int num0, den0, num1, den1;
	  num1= 28322 * n1 * (8 >> (m1>>6));
	  
	  num0= 25175 * n0 * (8 >> (m0>>6));
	  ...
	  f1 = num1/den1;
#ifdef XRIPDEBUG
	  ErrorF("f0 = %d/%d = %d f1 = %d/%d = %d\n", (int) num0, (int) den0,(int) f0, (int) num1, (int) den1, (int) f1);	  
#endif
      }
#ifdef XRIPDEBUG
      ErrorF("m0 = %d m1 = %d n0 = %d n1 = %d\n", m0,m1,n0,n1);
#endif
	 ...
	 if (fdiff < f0/20) {
	    ...
	    if ((int)(f/200) == 16000/200)
	       ...
	       s3InfoRec.s3RefClk = (f0+f1)/2;
	 } else {
	     ErrorF("fdiff is too great (%d limit %d)\n", fdiff, f0/20);
	 }
      }
      else {
	 ...
	 if (mclk)
	    ErrorF(" (MCLK %d KHz)", mclk);
	 ErrorF("\n");	
	 ErrorF("%s %s: with refclock %d KHz (probed %d & %d)\n",
		refclock_probed,s3InfoRec.name,s3InfoRec.s3RefClk,
		(int) f0,(int) f1);
      }
      numClocks = 3;
      if (!s3InfoRec.s3MClk)
	 ...
	 ErrorF("%s %s: Using Sierra SC11412 programmable clock\n",
      ...
      if (xf86Verbose)
	 ErrorF("%s %s: Using %s programmable clock (MCLK %d MHz)\n"
		,clockchip_probed, s3InfoRec.name
		,OFLG_ISSET(CLOCK_OPTION_ICS5342, &s3InfoRec.clockOptions)
	 ...
	 if (xf86Verbose)
	    ErrorF("%s %s: using specified MCLK value of %d KHz for DRAM timings\n",
		   XCONFIG_GIVEN, s3InfoRec.name, s3InfoRec.s3MClk);
      }
      else {
	 s3InfoRec.s3MClk = mclk;
      }
      ...
      mclk = ((1431818 * (m+2)) / (n1+2) / (1 << n2) + 50) / 100;
      if (xf86Verbose)
	 ErrorF("%s %s: Using Trio32/64 programmable clock (MCLK %d MHz)\n"
		,clockchip_probed, s3InfoRec.name
		,mclk);
      if (s3InfoRec.s3MClk > 0) {
	 if (xf86Verbose)
	    ErrorF("%s %s: using specified MCLK value of %d MHz for DRAM timings\n",
		   XCONFIG_GIVEN, s3InfoRec.name, s3InfoRec.s3MClk );
      }
      else {
	 s3InfoRec.s3MClk = mclk;
	 ...
	 if (i%16 == 15) ErrorF ("\n");
      ...
      if (xf86Verbose)
	 ErrorF("%s %s: Using STG1703 programmable clock(MCLK%d %02x %02x %d MHz)\n",
		XCONFIG_GIVEN, s3InfoRec.name, mi, ml,mh, mclk);
      if (s3InfoRec.s3MClk > 0) {
	 if (xf86Verbose)
	    ErrorF("%s %s: using specified MCLK value of %d MHz for DRAM timings\n",
		   XCONFIG_GIVEN, s3InfoRec.name, s3InfoRec.s3MClk );
   ...
   if (xf86Verbose) {
	 ...
	 for (j = 0; j < s3InfoRec.clocks; j++) {
	    ...
	    ErrorF(" %d", s3InfoRec.clock[j] / 1000);
         }
         ErrorF("\n");
      } 
   }
...
/* XXX NAS */
	 if (1||!pMode->PrivSize || !pMode->Private) {
...

{
   ...
   default:
	 ...
	 if (freq < 20000) {
	    ErrorF("%s %s: Specified dot clock (%d) too low for TI 3025",
		   XCONFIG_PROBED, s3InfoRec.name, freq );
	    result = FALSE;
	    break;
	 }
...

{
   ...
   default:
	 ...
	 /* XXXX Check this elsewhere */
	 if (freq < 13750) {
	    ErrorF("%s %s: Specified dot clock (%d) too low for TI 3026",
		   XCONFIG_PROBED, s3InfoRec.name, freq);
	    result = FALSE;
	    break;
	 }
...

{
   ...
   default:
	 ...
	 /* XXXX Check this elsewhere */
	 if (freq < 16250) {
	    ErrorF("%s %s: Specified dot clock (%d) too low for IBM RGB52x",
		   XCONFIG_PROBED, s3InfoRec.name, freq );
	    result = FALSE;
	    break;
	 }
...

{
   ...
   default:
	 ...
	 if (freq < 8500 || freq > 135000) {
	    ErrorF("%s %s: Specified dot clock (%d) out of range for Chrontel 8391",
		   XCONFIG_PROBED, s3InfoRec.name, freq );
	    result = FALSE;
	    break;
	 }
...

{
   ...
   default:
	 ...
	 /* XXXX Check this elsewhere */
	 if (freq < 8500 || freq > 135000) {
	    ErrorF("%s %s: Specified dot clock (%d) out of range for STG1703",
		   XCONFIG_PROBED, s3InfoRec.name, freq );
	    result = FALSE;
	    break;
	 }
