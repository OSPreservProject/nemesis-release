...
void main()
{
   ...
   printf("Ser.No. %c-%04ld.%03ld.%03ld\n",
	  ...
	  (serno & 0x1ffff) / 1000,
	  (serno & 0x1ffff) % 1000);
   printf("max pixel clock  %d KHz\n"
	  ,(eedata->max_pixclock,eedata->max_pixclock*4));
   printf("max memory clock %d KHz\n"
	  ,(eedata->max_memclock,eedata->max_memclock*4));
   ...
   for (i= 26; i<ndata-9; i+=9) {
	 ...
	 printf("\"%dx%dx%d\" \t %d   %4d %4d %4d %4d   %4d %4d %4d %4d\n"
