...
static inline int get_input(unsigned char* bp, unsigned int size)
{
	...
	int size_cut;
	int reached_end;
	if (size + in_file_position > in_file_size) {
	    size_cut = in_file_size - in_file_position;
	    reached_end = 1;
	} else {
	    size_cut = size;
	    reached_end = 0;
	}
	memcpy(bp, in_file_copy + in_file_position, size_cut);
	in_file_position += size_cut;
	if (reached_end) return GETHDR_EOF;
	return 0;
