...
#ifdef CLOCKDEBUG
      if (mode->Flags & V_DBLCLK) {
	 ErrorF("Setting clock doubler in s3Init(), freq = %d\n",
		s3InfoRec.clock[mode->Clock]);
      }
#endif
   ...
   if (S3_801_928_SERIES(s3ChipId) || S3_964_SERIES(s3ChipId)) {
	...
	if (s3PixelMultiplexing)
	 ...
	 if (s3InfoRec.videoRam < 2048 || S3_TRIOxx_SERIES(s3ChipId))
	    clock *= 2;
#if 1
         m = (((mclk * 64610 + 1479843)/(1000*clock + 39000000)) - 21159300)/1000
;
#else 
...
InitLUT()
{
   ...
   for (i=0; i<256; i++) {
       outb(DAC_DATA, i & 0xe0);
       outb(DAC_DATA, (i & 0x1c)<<3);
       outb(DAC_DATA, (i & 0x03)<<6);
   }
