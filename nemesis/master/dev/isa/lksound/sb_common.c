...
sb_devc *detected_devc = NULL;	/* For communication from probe to init */
...
int intlog_position = 0;
#endif
Au_Rec aurec;
Time_clp time_cl;

#define SCHEME_CHARGING_ON
#undef SCHEME_LASTTIMES

#ifdef SCHEME_LASTTIMES
static void interrupt_action(void *stv, const kinfo_t *kinfo, uint64_t now) {
    static uint64_t lasttimes[NUMLASTTIMES];
    static int32_t history_repeating = 0;

    if (history_repeating == 1) {
	if (aurec.length * NUMLASTTIMES) {
	    aurec.master_sample_period = (now - lasttimes[interrupt_count & LASTTIMES_MASK])/(NUMLASTTIMES *  (aurec.length));
	    history_repeating = 2;
	}
    } 
    if (history_repeating == 0) {
	if (interrupt_count== STARTUP_DELAY_COUNT) {
	    history_repeating = 1;
	    interrupt_count = 0;
	    }
    }
    /* continue to build up the lasttimes buffer */
    lasttimes[interrupt_count & LASTTIMES_MASK] = now;
    
    /* record only the first interrupt in to the timestamp buffer */
    
    if (interrupt_count==0) {
	aurec.master_timestamp = now;
    } else {
#if 0
	aurec.master_timestamp = now - (interrupt_count * aurec.master_sample_period * aurec.length);
#endif
    }
}
#endif

#ifdef SCHEME_CHARGING_ON
static void interrupt_action(void *stv, const kinfo_t *kinfo, uint64_t now) {
    static uint64_t last = 0;

    
    aurec.master_timestamp = now;
    aurec.master_location += aurec.length;
    if (aurec.length)
	aurec.master_sample_period = ((now-last) / aurec.length);
    last = now;
}
#endif

static void sbintr(void *stv, const kinfo_t *kinfo) {
    uint64_t now;
    int status;
    ...
    sb_devc *devc = stv;
    now = Time$Now(time_cl);
    devc->irq_ok = 1;
    /* take action based on interrupt occuring */
    interrupt_action(stv, kinfo, now);

#ifdef LOWLOG_INTERRUPTS
    /* log interrupt occuring */
    ((uint64_t *)((uint32_t) (192 * 1024)))[intlog_position] = now;
    intlog_position++;
    if (intlog_position > 8192) intlog_position = 0;
#endif

	/* increment interrupt count, anyway */
	interrupt_count++;


	{
	    int *int_mask;
	    int_mask = kinfo->int_mask;
	    (*int_mask) &= ~(1<<(devc->irq));	    
	    if (devc->irq < IRQ_V_PIC2) {
...
#if 0 /* don't bother do any of the old chaff */
 ...
 */
	...
	{			/* IRQ setup */
		...
		if (devc->major == 4 && devc->minor <= 11 )	/* Won't work */ {
			devc->irq_ok = 1;
			DDB(printk("Skipping interrupt test\n"));
		}
		else
		{
			int n;
			DDB(printk("Doing interrupt test\n"));
