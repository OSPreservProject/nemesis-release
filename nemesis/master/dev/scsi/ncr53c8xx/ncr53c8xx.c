...
/* #define SCSI_NCR_DEBUG_FLAGS (DEBUG_TINY|DEBUG_RESULT)  */
...
/* Below are the functions provided by this file in Nemesis */
PUBLIC struct Scsi_Host **ncr53c8xx_init();
...
/* XXX PRB Pretend to be 2.0.0 */
#define LINUX_VERSION_CODE LinuxVersionCode(2,0,0)
...
/* Fake out kernel SCSI datastructures */
#include "fake.h"
#include "ncr53c8xx.h"
...
#if defined(SCSI_NCR_IOMAPPED) || defined(__alpha__) || defined(__ix86__)
#define NCR_IOMAPPED
#else

#error NCR trying to use memory mapped IO on Nemesis.
...
#endif /* defined(SCSI_NCR_IOMAPPED) || defined(__alpha__) */



#else /* i.e LINUX_VERSION_CODE < LinuxVersionCode(1,3,0)  */
...
#warning urk using null vtophys
...
/*
** SMH: on Nemesis, we don't have a kmalloc (which was in the original 
** driver); instead we use a special heap, "k_heap", which is unfortunately
** a global variable ;-(
*/
    
#define KHEAPSZ (16384)
static Heap_clp k_heap = NULL;

static INLINE void *m_alloc(int size)
{
    void *ptr = (void *) Heap$Malloc(k_heap, size);
    return ptr;
}

static INLINE void m_free(void *ptr, int size) 
{ 
    Heap$Free(k_heap, ptr);
}
...
#define ChipVersion	((np)->revision_id & 0xf0)
    ...
    */
    ...
    /* 
     *   PRB: Nemesis interrupt stub state 
     */
    struct {
...
static struct Scsi_Host *ncr_attach (Scsi_Host_Template *tpnt, int unit, 
				     u_short device_id, u_char revision_id,
				     int chip, u_int base, u_int io_port, 
 ...
 */
static void *script_kvars[SCRIPT_KVAR_LAST] =
{ &(INFO_PAGE.now) /* (void *)&jiffies*/ };
...
*/
    ...
    */
    SCR_COPY (sizeof ( Time_ns /*XXX PRB u_long*/)),
    KVAR(SCRIPT_KVAR_JIFFIES),
...
void ncr_script_fill (struct script * scr)
{
    ...
    *p++ =PADDR (no_data);
    *p++ =SCR_COPY (sizeof ( Time_ns /* XXX PRB u_long */));
	...
	*/
	...
	if (relocs) {
		...
		case 0:
		    ...
		    /* fall through */
		default:
		    panic("ncr_script_copy_and_bind: weird relocation.\n");
		    break;
		}
...
/*****************************************/
/* XXX Note that these need to be contiguous */

typedef struct {
    struct Scsi_Host thehost;
    struct host_data thehostdata;
} percontroller;

static struct Scsi_Host *
scsi_register(Scsi_Host_Template *tpnt, int nb)
{
    percontroller *pc;
...
#define ETRC(_x) 
	...
	Mem_PMemDesc pm;
	Frames_clp f;
	word_t reqd = (sizeof(*pc) + FRAME_SIZE - 1) & ~(FRAME_SIZE-1);
	addr_t pbase, vbase; 
	Stretch_clp str;
	Stretch_Size sz; 

	ETRC(eprintf("ncr: require 0x%x bytes for controller state.\n", reqd));

	/* Get a contiguous region of nailed memory for hostdata/script */
	ETRC(eprintf("ncr: finding frames...\n"));
	f = NAME_FIND("sys>Frames", Frames_clp);
	pbase = Frames$AllocRange(f, reqd, FRAME_WIDTH, ANY_ADDRESS, 
				  SET_ELEM(Mem_Attr_DMA)); 

	ETRC(eprintf("ncr: allocated %x bytes at [phys] %p\n", reqd, pbase));
	pm.start_addr  = pbase; 
	pm.frame_width = FRAME_WIDTH; 
	pm.nframes     = (word_t)reqd >> FRAME_WIDTH; 
	pm.attr        = SET_ELEM(Mem_Attr_DMA);

	ETRC(eprintf("ncr: about to created a new mapped stretch.\n"));
	str = StretchAllocator$NewAt(Pvs(salloc), reqd, 0, 
				     ANY_ADDRESS, 0, &pm);
	vbase = Stretch$Range(str, &sz);
	ETRC(eprintf("ncr: allocated %x bytes at [virt] %p\n", sz, vbase));


	ETRC(eprintf("ncr: got it. now binding to a stretch driver.\n"));
	/* XXX should use a nailed stretch driver */
	StretchDriver$Bind(Pvs(sdriver), str, PAGE_WIDTH);

	pc = vbase; 
    }
#else
    pc = Heap$Malloc(Pvs(heap), sizeof(percontroller));
    if (pc == NULL) RAISE_Heap$NoMemory();
#endif

    memset(pc, 0, sizeof(percontroller));
    return (struct Scsi_Host *)pc;
}

static void
scsi_unregister(struct Scsi_Host *sh)
{
  Heap$Free(Pvs(heap), sh);
}
...
static int interpret_sense(Scsi_Cmnd *sc)
{
    struct scsi_sense_data_new *sense = &sc->sense_buffer;
    int	key;
    int	silent = 0;
    int device = 0;
	...
	int i;
	uchar_t *buf = sense;
	for (i = 0; i < sizeof(struct scsi_sense_data_new); i++) {
	    printf("%02x ", buf[i]);
	    if ((i & 15) == 15) printf("\n");
	}
    }

/* XXX DEC RZ26 appears not to set the valid bit, but the error code is 0x70
   I haven't a clue what's supposed to happen!  Somebody who understands
   bloody disk drives can fix this!!!
   */
#if 0
    if (sense->ext.extended.valid != 1)
    {
	DB(fprintf(stderr,"Warning: interpret_sense: unknown format %d.\n",
		   sense->ext.extended.error_code));
	if (sense->ext.extended.error_code == 0x0)
	{
	    /* XXX- some device, eg. Tamarack scanner actually return
	       this code and actually it means 0x70
	       Try this anyway. */
	    sense->ext.extended.error_code = 0x70;
	}
	else
	    return(EIO);
    }
    ...
    if (sense->ext.extended.error_code == 0x70)
    {
	...
	case	0x1:
	    if(!silent)
	    {
		fprintf(stderr,"scsi disk %d: soft error(corrected) ",device); 
		if(sense->ext.extended.valid)
		{
		    fprintf(stderr,"block no. %02x%02x%02x%02x (hex)",
	...
	case	0x2:
	    if(!silent)
		fprintf(stderr,"scsi disk %d: not ready\n ",device); 
	    return(ENODEV);
	case	0x3:
	    if(!silent)
	    {
		fprintf(stderr,"scsi disk %d: medium error ", device); 
		if(sense->ext.extended.valid)
		{
		    fprintf(stderr,"block no. %02x%02x%02x%02x (hex)",
			    (sense->ext.extended.info[0] <<24),
	...
	case	0x4:
	    if(!silent)
		fprintf(stderr,"scsi disk %d: non-media hardware failure\n ",
			device); 
	    return(EIO);
	case	0x5:
	    if(!silent)
		fprintf(stderr,"scsi disk %d: illegal request\n ",
			device); 
	    return(EINVAL);
	case	0x6:
	    if(!silent)
		fprintf(stderr,"scsi disk %d: Unit attention.\n ",
			device); 
	    return(0);
	case	0x7:
	    if(!silent)
	    {
		fprintf(stderr,"scsi disk %d: attempted protection violation ",
			device); 
		if(sense->ext.extended.valid)
		{
		    fprintf(stderr,"block no. %02x%02x%02x%02x (hex)",
	...
	case	0x8:
	    if(!silent)
	    {
		fprintf(stderr,"scsi disk %d: block wrong state (worm)\n ",
			device); 
		if(sense->ext.extended.valid)
		{
		    fprintf(stderr,"block no. %02x%02x%02x%02x (hex)",
	...
	case	0x9:
	    if(!silent)
		fprintf(stderr,"scsi disk %d: vendor unique\n",
			device); 
	    return(EIO);
	case	0xa:
	    if(!silent)
		fprintf(stderr,"scsi disk %d: copy aborted\n ",
			device); 
	    return(EIO);
	case	0xb:
	    if(!silent)
		fprintf(stderr,"scsi disk %d: command aborted\n ",
			device); 
	    return(EIO);
	case	0xc:
	    if(!silent)
	    {
		fprintf(stderr,"scsi disk %d: search returned\n ",
			device); 
		if(sense->ext.extended.valid)
		{
		    fprintf(stderr,"block no. %02x%02x%02x%02x (hex)",
	...
	case	0xd:
	    if(!silent)
		fprintf(stderr,"scsi disk %d: volume overflow\n ",
			device); 
	    return(ENOSPC);
	case	0xe:
	    if(!silent)
	    {
		fprintf(stderr,"scsi disk %d: verify miscompare\n ",
			device); 
		if(sense->ext.extended.valid)
		{
		    fprintf(stderr,"block no. %02x%02x%02x%02x (hex)",
	...
	case	0xf:
	    if(!silent)
		fprintf(stderr,"scsi disk %d: unknown error key\n ",
			device); 
	    return(EIO);
	}
    }
    ...
    {
	DB(fprintf(stderr,"interpret_sense: unsupported error_code %02x.\n",
		   sense->ext.extended.error_code));
	return(EIO);
    }

    /* never reach here */
    return(0); /* just to silent the compiler */
}

static void InterruptThread_m (	ncb_p np)
{
    Event_Val cur, waitfor = 1;

    TRC(printk("%s: Interrupt thread entered.\n", ncr_name(np)));
	...
	cur = EC_AWAIT_UNTIL(np->stub.event, waitfor, 
			     NOW() + SCSI_NCR_TIMER_INTERVAL);

	if (cur > waitfor)  
	    printf("************* TOOK %d INTERRUPTS\n", (cur - waitfor) + 1);

	TRC(printk("%s: Interrupt thread woken.\n", ncr_name(np)));
...
static struct Scsi_Host *ncr_attach (Scsi_Host_Template *tpnt, 
				     int unit, u_short device_id,
				     u_char revision_id, int chip, 
				     u_int base, u_int io_port, 
				     int irq, int bus, u_char device_fn)

{
...
#if 0  /* don't actually use flags in save/restore */
    u_long flags = 0;
#endif

    TRC(printf("ncr_attach: unit=%d chip=%d base=%lx, io_port=%lx, irq=%d\n", 
    ...
    **  XXX SMH: for now we don't bother - just use IO cycles. 
    */
...
#ifndef NCR_IOMAPPED
    np->reg_remapped = (struct ncr_reg *) 
	remap_pci_mem((u_long) base, (u_long) 128);
    if (!np->reg_remapped) {
	printf("%s: can't map memory mapped IO region\n", 
	       ncr_name(np));
	np->use_mmio = 0;
    }
    TRC(printf("%s: using memory mapped IO at virtual address 0x%lx\n", 
    ...
    */
    /* XXX deal with externs in ucode script */
...
retry_chip_init:
    ...
    */
...
#ifndef NCR_IOMAPPED
	if (np->use_mmio) {
	    printf("%s: cache misconfigured, retrying with "
		   "IO mapped at 0x%lx\n", ncr_name(np), 
		   (u_long) np->port);
	    np->use_mmio = 0;
    ...
    /* PRB: Interrupts the Nemesis way... :-) */
    {  
	Interrupt_clp       ipt;
	Interrupt_AllocCode rc;
	Thread_clp          intt;
	/* Externs */
	extern void _stub(void *st);        /* Interrupt stub code */
	
	/* Stub state */
	np->stub.dcb   = RO(Pvs(vp));
	np->stub.ep    = Events$AllocChannel (Pvs(evs));
	np->stub.event = EC_NEW();
	
	/* XXX PRB Rather than using an event count to block on, 
	   we could simply attach an EntryNotify closure to the
	   Channel_EndPoint */
	Events$Attach(Pvs(evs), np->stub.event, 
		     np->stub.ep, Channel_EPType_RX);
	TRC(printk("%s: channel (%lx,%lx)\n", 
		    ncr_name(np), np->stub.dcb, np->stub.ep )); 
	
	TRC(printk("%s: Grabbing interrupt allocator\n", ncr_name(np)));
	ipt = IDC_OPEN("sys>InterruptOffer", Interrupt_clp);
	
	TRC(printk("%s: Allocating interrupt.\n", ncr_name(np)));
	rc = Interrupt$Allocate(ipt, np->irq, _generic_stub, &np->stub); 
	TRC(printk("%s: allocate returns %x\n", ncr_name(np), rc));
	
	/* Fork the interrupt thread */
	TRC(printk("%s: Forking Interrupt Thread\n", ncr_name(np)));
	intt = Threads$Fork(Pvs(thds), InterruptThread_m, np, 0 ); 
    }
    ...
    */
#if 0 /* XXX PRB: Timeout daemon stupid! */
    ncr_timeout (np);
    np->lasttime=0;
#endif 0
...
/*	Scsi_Device        *device    = cmd->device; */
    ...
    */
    ...
    if(cp->cmd->request_bufflen > 0) {

	/* SMH: we only setup the segs if there is a non-empty buffer */
    ...
    */
    ...
    else cp->tlimit= jiffies + 3600 * HZ; /* No timeout=one hour */
    cp->magic		= CCB_MAGIC;
...
/*	Scsi_Device        *device    = cmd->device; */
    ...
    printk("ncr_abort_command called! urk! cmd=%p\n", cmd);
    save_flags(flags); cli();
/*
...
static int ncr_detach(ncb_p np, int irq)
{
    ...
    TRC(printf("%s: releasing host resources\n", ncr_name(np)));
...
#   endif
    ...
    */
    ...
    TRC(printf("%s: host resources successfully released\n", ncr_name(np)));
...
void ncr_complete (ncb_p np, ccb_p cp)
{
    ...
    */
	...
	cmd->scsi_done (cmd);
      ...
      printk("Urk! ncr_complete, command at %p: scsi_done is NULL!\n", 
	      cmd);
    }
}
...
void ncr_init (ncb_p np, char * msg, u_long code)
{
    ...
    */
	...
	if (ChipDevice == PCI_DEVICE_ID_NCR_53C815) {
		...
		if ((ChipDevice == PCI_DEVICE_ID_NCR_53C810 && 
		     ChipVersion >= 0x10) ||
		    ...
		    if ((ChipDevice == PCI_DEVICE_ID_NCR_53C825 && 
			 ChipVersion >= 0x10) ||
			...
			OUTB(nc_ctest3, 0x01);	/* Set write and invalidate */
			/* Cache line size enable and 700 comp*/
			OUTB(nc_dcntl, 0x81);	
#endif
		    } else {
			/**	OTHERS				**/
...
void ncr_exception (ncb_p np)
{
    ...
    */
	...
	printf ("<%d|%x:%x|%x:%x>",
		...
		(unsigned)INL(nc_dsp),
		(unsigned)INL(nc_dbc));
    if ((dstat==DFE) && (sist==PAR)) { printf("DFE/PAR\n"); return; }
...
*/
    ...
    */
	...
	/* XXX PRB No timer del_timer(&np->timer); */
...
static void ncr_int_ma (ncb_p np)
{
    ...
    if (DEBUG_FLAGS & DEBUG_TINY) printf ("M");
    ...
    */
    ...
    if (!cp) {
	printf ("%s: SCSI phase error fixup: CCB already "
		"dequeued (0x%08lx)\n", 
...
static	int	ncr_scatter(ccb_p cp, Scsi_Cmnd *cmd)
{
    ...
    */
	...
	for (segment = 0 ; segment < cmd->use_sg ; segment++) {
	    ...
	    if (DEBUG_FLAGS & DEBUG_SCATTER)
		printf("ncr53c8xx: re-scattering physical=0x%x "
		       "size=%d chunk=%d.\n",
		       (unsigned) segaddr, (int) datalen, (int) chunk);
...
static int ncr_snooptest (struct ncb* np)
{
    ...
    */
	...
	printf("pc is %p, NCB_SCRIPT_PHYS(np,snoopend)+8 is %p\n", 
	       pc, NCB_SCRIPT_PHYS(np,snoopend)+8);
	return (0x40);
    };
    /*
    **	Show results.
...
static struct Scsi_Host *ncr53c8xx_pci_init(PCIBios_clp pcibios, 
					    Scsi_Host_Template *tpnt, 
					    int unit, int board, int chip,
					    uchar bus, uchar device_fn, 
					    int options);
static struct Scsi_Host **ncr53c8xx_detect(Scsi_Host_Template *tpnt,
					   int *nboards);
					   ...
					   int *nboards)
{
    ...
    NOCLOBBER PCIBios_clp pcibios;
    struct Scsi_Host ** NOCLOBBER shs;
    struct Scsi_Host *sh;
    int finddevwrapper(        PCIBios_cl      *self,
        uint16_t        vendor  /* IN */,
        uint16_t        device  /* IN */,
        uint32_t        index   /* IN */
   /* RETURNS */,
	...
	int r;
	ENTER_KERNEL_CRITICAL_SECTION();
	r=PCIBios$FindDevice(self, vendor, device, index, bus, devfn);
	LEAVE_KERNEL_CRITICAL_SECTION();
	return r;
    }
    ...
    *nboards = 0;
	...
	TRC(printk("ncr53c8xx_detect: PCIBios at %p\n", pcibios));
    } 
    CATCH_Context$NotFound(name) {
	printk("ncr53c8xx_detect: PCIBios (%s) not present.\n", name);
	return NULL;
    }
    ENDTRY;
	...
	for (pci_index = 0; !finddevwrapper(
	    pcibios, PCI_VENDOR_ID_NCR, 
				 pci_chip_ids[i].pci_device_id, 
				 pci_index, &pci_bus, 
				 &pci_device_fn);
	     ++pci_index)
	    if ((sh = ncr53c8xx_pci_init(pcibios, tpnt, *nboards, 0, 
					 pci_chip_ids[i].chip, 
					 pci_bus, pci_device_fn, 
					 /* no options */ 0)) != NULL) {
	      if (*nboards == 0) 
		shs = Heap$Malloc(Pvs(heap), sizeof(struct Scsi_Host *));
	      else
		shs = Heap$Realloc(Pvs(heap), shs, (*nboards + 1) *
				   sizeof(struct Scsi_Host *));
	      if (shs == NULL)
		RAISE_Heap$NoMemory();
	      shs[*nboards] = sh;
	      (*nboards)++;
					    ...
					    int options)
{
    ...
    ENTER_KERNEL_CRITICAL_SECTION();
    error = PCIBios$ReadConfigWord( pcibios, bus, device_fn, 
				    PCI_VENDOR_ID, &vendor_id) ||
	PCIBios$ReadConfigWord( pcibios, bus, device_fn, 
				PCI_DEVICE_ID, &device_id) ||
	PCIBios$ReadConfigWord( pcibios, bus, device_fn, 
				PCI_COMMAND, &command)   ||
	PCIBios$ReadConfigDWord(pcibios, bus, device_fn, 
				PCI_BASE_ADDRESS_0, &io_port)   || 
	PCIBios$ReadConfigDWord(pcibios, bus, device_fn, 
				PCI_BASE_ADDRESS_1, &base)      ||
	PCIBios$ReadConfigByte (pcibios, bus, device_fn, 
				PCI_CLASS_REVISION, &revision)  ||
	PCIBios$ReadConfigByte (pcibios, bus, device_fn, 
				PCI_INTERRUPT_LINE, &irq);
    LEAVE_KERNEL_CRITICAL_SECTION();
    if (error) {
	printk("ncr53c8xx : error %x not initializing due to error "
	       "reading configuration space\n", error);
	return NULL;
    }

    if (vendor_id != PCI_VENDOR_ID_NCR) {
	printk("ncr53c8xx : not initializing, 0x%04x is not NCR "
	       "vendor ID\n", (int) vendor_id);
	return NULL;
    }
	...
	if ((io_port & 3) != 1) {
	    printk("ncr53c8xx : disabling I/O mapping since base "
		   " address 0 (0x%x)\n"
	...
	if ((base & PCI_BASE_ADDRESS_SPACE) != PCI_BASE_ADDRESS_SPACE_MEMORY) {
	    printk("ncr53c8xx : disabling memory mapping since base "
		   "address 1\n"
    ...
    if (!io_port && !base) {
	printk("ncr53c8xx : not initializing, both I/O and memory "
	       "mappings disabled\n");
	return NULL;
    }
	
    if (!(command & PCI_COMMAND_MASTER)) {
	printk ("ncr53c8xx : not initializing, BUS MASTERING "
		"was disabled\n");
	return NULL;
    }
...
/*
** Nemesis 'entry point'. Oh yeah.
*/
struct Scsi_Host **ncr53c8xx_init(int *nboards)
{
  ...
  Pvs(out) = Pvs(err); 
	...
	ETRC(eprintf("ncr: sorting out %lx bytes nailed heap.\n", reqd));

	ETRC(eprintf("ncr: finding frames...\n"));
	f = NAME_FIND("sys>Frames", Frames_clp);
	pbase = Frames$AllocRange(f, reqd, FRAME_WIDTH, ANY_ADDRESS, 0); 
	...
	k_heap = HeapMod$NewRaw(hmod, vbase, reqd);
	ETRC(eprintf("ncr: got nailed heap at %p\n", k_heap));
    }
#else
    /* Pre MEMSYS_EXPT, we can just use our standard heap */
    k_heap = Pvs(heap);
#endif
    
  shs = ncr53c8xx_detect((Scsi_Host_Template *)BADPTR, nboards);
  TRC(printf("ncr53c8xx: detected %d boards.\n", *nboards)); 
  return shs;
}
...
static void process_waiting_list(ncb_p np, int sts)
{
	...
	if (sts != DID_OK) {
		...
		wcmd->scsi_done(wcmd);
	      ...
	      printk("process_waiting_list: command at %p, "
			"scsi_done is NULL!\n", wcmd);
	    }
	}
    }
...
/* XXX PRB */
#undef SCSI_NCR_PROC_INFO_SUPPORT
